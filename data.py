...
class SharedData:
    def __init__(self):
        ...
        self.rh_dict = self.load_rh_dict()
        self.rr_dict = self.load_rr_dict()
        self.jh_dict = self.load_jh_dict()
        self.jr_dict = self.load_jr_dict()
        self.rhdata = self.load_rhdata()
        self.rrdata = self.load_rrdata()
        # self.jhdata = self.load_jhdata()
        # self.jrdata = self.load_jrdata()
    
    def load_title(self):
        return ['燃油增压泵滑油端', '燃油增压泵燃油端', '加力燃油泵滑油端', '加力燃油泵燃油端']
    def load_rh_dict(self):
        return {'压力':('0.1MPa', '0.3MPa', '0.5MPa'),
                ...
                '喷淋角度':('38°', '35°', '41°'),
                '喷淋管数':('1', '2')}
    def load_rr_dict(self):
        return {'压力':('0.1MPa', '0.2MPa', '0.3MPa'),
                '转速':('6630rpm', '8565rpm', '9210rpm'),
                '织构':('无织构', '人字槽', '螺旋槽'),
                '出口直径':('2mm', '3mm')}
    def load_jh_dict(self):
        return {'转速':('18835rpm', '21255rpm', '22600rpm', '23950rpm', '25560rpm', '26906rpm'),
                '压力':('0.25MPa', '0.3MPa', '0.35MPa'),
                '入口管径':('0.7mm', '0.8mm'),
                '织构':('无织构', '激光脸(新)'),
                '喷淋角度':('14.5°', '11.5°', '17.5°')}
    def load_jr_dict(self):
        return {'转速':('18835rpm', '21255rpm', '22600rpm', '23950rpm', '25560rpm', '26906rpm'),
                '流速':('1900-2000ml/min', '2300-2400ml/min'),
                '织构':('无织构', '激光脸(新)'),
                '进口管径':('1mm', '1.5mm')}
    def load_rhdata(self):
        return {
